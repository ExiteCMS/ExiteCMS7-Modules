...
//
$ad_locations = array($locale['ads450'], $locale['ads451'], $locale['ads452'], $locale['ads453'], $locale['ads454'], $locale['ads455']);
$ad_dimensions = array("160x65", "468x60", "468x60", "468x60", "468x60", "468x60");
...
if (isset($_POST['expire'])) {
	$result = dbquery("UPDATE ".$db_prefix."advertising SET adverts_expired = '1' WHERE adverts_id = '".$adverts_id."'");
	$action = "list";
}

// activate requested
if (isset($_POST['activate'])) {
	$result = dbquery("UPDATE ".$db_prefix."advertising SET adverts_expired = '0' WHERE adverts_id = '".$adverts_id."'");
	$action = "list";
}
		...
		if (isset($_POST['newid']) && isNum($_POST['newid'])) {
			...
			if (dbrows($result)) {
				...
				if (isset($_POST['adverts_id']) && isNum($_POST['adverts_id'])) {
					$result = dbquery("SELECT * FROM ".$db_prefix."advertising WHERE adverts_id = '".$_POST['adverts_id']."'");
					if (dbrows($result)) {
						...
						$result = dbquery("UPDATE ".$db_prefix."advertising SET adverts_userid = '".$_POST['newid']."' WHERE adverts_id = '".$_POST['adverts_id']."'");
...
if ($action == "delclientconf") {
	if (isset($_POST['yes'])) {
		...
		if ($data = dbarray($result)) {
			...
			$result = dbquery("DELETE FROM ".$db_prefix."advertising WHERE adverts_userid = '$id'");
...
if ($action == "images") {
	...
	foreach($image_list as $img) {
		...
		$result = dbquery("SELECT * FROM ".$db_prefix."advertising WHERE adverts_image = '".$img."'");
		$used = dbrows($result);
...
if ($action == "list") {
	...
	while ($data = dbarray($result)) {
		...
		$result2 = dbquery("SELECT * FROM ".$db_prefix."advertising WHERE adverts_expired = '0' AND adverts_userid = '".$data['user_id']."' ORDER BY adverts_id DESC");
		$ads1 = array();
		if (dbrows($result2)) {
