...
if ($settings['dlstats_remote']) {
	$result = dbquery("UPDATE ".$db_prefix."downloads SET download_count = 0");
}
			...
			if (count($logrec) == 5 && isNum($logrec[0]) && isIP($logrec[2]) && isNum($logrec[3])) {
				...
				if (count($logrec[1]) == 3 && isNum($logrec[1][0]) && isNum($logrec[1][1]) && isNum($logrec[1][2])) {
					...
					if (mysqli_affected_rows() == 0) {
						...
						$result2 = dbquery("INSERT INTO ".$db_prefix."dlstats_ips (dlsi_ip, dlsi_ccode, dlsi_onmap, dlsi_counter) VALUES ('".$logrec[2]."', '".$logrec['cc']."', '".$logrec['on_map']."', 1) ON DUPLICATE KEY UPDATE dlsi_counter = dlsi_counter + 1".($logrec['on_map'] == 1 ? ", dlsi_onmap = 1" : ""));
						// update fhe File statistics
						$result2 = dbquery("INSERT INTO ".$db_prefix."dlstats_files (dlsf_file, dlsf_success, dlsf_counter) VALUES ('".$logrec[4]."', '".$logrec[3]."', 1) ON DUPLICATE KEY UPDATE dlsf_counter = dlsf_counter + 1");
						// now update the download details for this record
						// get the dlsi_id for this IP
						$result2 = dbquery("SELECT dlsi_id FROM ".$db_prefix."dlstats_ips WHERE dlsi_ip = '".$logrec[2]."' LIMIT 1");
						if (dbrows($result2)) {
							$data2 = dbarray($result2);
							// get the dlsf_id for this file
							$result3 = dbquery("SELECT dlsf_id FROM ".$db_prefix."dlstats_files WHERE dlsf_file = '".$logrec[4]."' LIMIT 1");
							if (dbrows($result3)) {
								$data3 = dbarray($result3);
								$result4 = dbquery("INSERT INTO ".$db_prefix."dlstats_file_ips (dlsi_id, dlsf_id, dlsfi_timestamp) VALUES ('".$data2['dlsi_id']."', '".$data3['dlsf_id']."', '".$logrec[1][2]."')");
						...
						if ($settings['dlstats_remote']) {
							...
							$result2 = dbquery("SELECT * FROM ".$db_prefix."downloads WHERE download_url LIKE '%".$logrec[4]."' AND download_external = 1");
							while ($data2 = dbar($result2)) {
