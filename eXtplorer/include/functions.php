...
function get_is_editable( $abs_item ) {		// is this file editable?
	...
	foreach($GLOBALS["editable_ext"] as $pat) if(@preg_match('~'.$pat.'~i',$abs_item)) return true;
...
function get_mime_type( $abs_item, $query) {	// get file's mimetype
	...
	if((function_exists("is_executable") &&
		...
		@preg_match('~'.$GLOBALS["super_mimes"]["exe"][2].'~i',$abs_item))
	{						// executable
...
function id_browser() {
	...
	if(preg_match('~Opera(/| )([0-9].[0-9]{1,2})~', $browser)) {
		...
		return 'KONQUEROR';
	} else if(preg_replace('~Mozilla/([0-9].[0-9]{1,2})~', $browser)) {
		return 'MOZILLA';
	} else {
		return 'OTHER';
	}
