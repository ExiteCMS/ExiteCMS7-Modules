		...
		if (isset($_POST['save'])) {
			...
			if ($variables['travel_mileage'] == "") {
				...
				if (!empty($variables['new_ident'])) {
					$result = dbquery("SELECT adr_id FROM ".$db_prefix."mileage_address WHERE adr_driver_id = '".$variables['travel_driver_id']."' AND adr_description = '".mysql_real_escape_string($variables['new_ident'])."'");
					if (dbrows($result)) {
				...
				if (isset($_POST['save_detour'])) {
					$result = dbquery("UPDATE ".$db_prefix."mileage_distance SET distance_detour = '".$variables['travel_detour']."', distance_detour_type = '".$variables['travel_detour_type']."', distance_detour_reason = '".mysql_real_escape_string($variables['travel_detour_reason'])."' WHERE distance_from = '".$variables['travel_from']."' AND distance_to = '".$variables['travel_to']."'");
				}
				if ($action == "add") {
					// add the new car
					$result = dbquery("INSERT INTO ".$db_prefix."mileage_travel (travel_car_id, travel_driver_id, travel_to, travel_type, travel_mileage, travel_date, travel_detour, travel_detour_type, travel_detour_reason, travel_details, travel_fuel, travel_fuel_type) VALUES ('".$variables['travel_car_id']."', '".$variables['travel_driver_id']."', '".$variables['travel_to']."', '".$variables['travel_type']."', '".$variables['travel_mileage']."', '".$variables['travel_date']."', '".$variables['travel_detour']."', '".$variables['travel_detour_type']."', '".mysql_real_escape_string($variables['travel_detour_reason'])."', '".mysql_real_escape_string($variables['travel_details'])."', '".$variables['travel_fuel']."', '".$variables['travel_fuel_type']."')");
					// return to the main panel with a success message
					$variables['message'] = $locale['eA_915'];
				} elseif ($action == "edit") {
					// update the trip
					$result = dbquery("UPDATE ".$db_prefix."mileage_travel SET travel_car_id = '".$variables['travel_car_id']."', travel_driver_id = '".$variables['travel_driver_id']."', travel_to = '".$variables['travel_to']."', travel_type = '".$variables['travel_type']."', travel_mileage = '".$variables['travel_mileage']."', travel_date = '".$variables['travel_date']."', travel_detour = '".$variables['travel_detour']."', travel_detour_type = '".$variables['travel_detour_type']."', travel_detour_reason = '".mysql_real_escape_string($variables['travel_detour_reason'])."', travel_details = '".mysql_real_escape_string($variables['travel_details'])."', travel_fuel = '".$variables['travel_fuel']."', travel_fuel_type = '".$variables['travel_fuel_type']."' WHERE travel_id = '".$travel_id."'");
					// return to the main panel with a success message
