...
{***************************************************************************}
{*                                                                         *}
{* Template for the mileage module 'print' function, standard layout       *}
{*                                                                         *}
{***************************************************************************}
{literal}<style type="text/css">
body 		{ font-family:Verdana,Tahoma,Arial,Sans-Serif;font-size:10pt; }
hr 			{ height:1px;color:#ccc; }
td			{ padding: 4px 2px 4px 4px; }
...
<table width='900px' class='tbl' cellpadding='0' cellspacing='0'>
	{section name=id loop=$trips}
		...
		{if $report_option == 3 && $trips[id].fuel.fuel.fuel|default:0 == 0}
			{* skip this line for report 3, when there is no refueling on this trip *}
		{else}
			<tr>
				...
				<td rowspan='{$rowspan}' class='{cycle values="tbl1,tbl2" advance=no}' align='right' style='white-space:nowrap'>
					{ssprintf format=$format var1=$trips[id].from.mileage}{$mileage_unit} &rsaquo; {ssprintf format=$format var1=$trips[id].to.mileage}{$mileage_unit}
				</td>
				<td rowspan='{$rowspan}'  class='{cycle values="tbl1,tbl2" advance=no}' width='1%' align='right' style='white-space:nowrap'>
					{ssprintf format=$format var1=$trips[id].trip.business}{$mileage_unit}
				</td>
				<td rowspan='{$rowspan}'  class='{cycle values="tbl1,tbl2" advance=no}' width='1%' align='right' style='white-space:nowrap;'>
					{ssprintf format=$format var1=$trips[id].trip.personal}{$mileage_unit}
				</td>
				...
				{if $trips[id].fuel.fuel.fuel|default:0 > 0}
					<tr>
						<td class='{cycle values="tbl1,tbl2" advance=no} dashtop' align='left'>
							...
							{if $report_option != 1 && $trips[id].fuel.fuel.range > 0}
		...
		{if $smarty.section.id.last}
			...
			<br />
			<table width='900px'>
				{if $report_option == 2 || $report_option == 3}
					<tr>
